#!/usr/bin/env python3 
import rclpy
from rclpy.node import Node
from nav_msgs.msg import OccupancyGrid, MapMetaData
from sensor_msgs.msg import LaserScan
from tf2_ros import Buffer, TransformListener, LookupException

class Pose: 
    def __init__(self, px=0, py=0):
        self.x = px
        self.y = py

#función global que devuelve la pos del robot en pos del mapa (creo)
def coordinatesToPose(px, py, map_info: MapMetaData):
    pose = Pose()
    pose.x = round(px - (map_info.origin.position.x) / map_info.resolution)
    pose.y = round(py - (map_info.origin.position.y) / map_info.resolution)
    return pose

#función que define si la posición del robot está fuera o no del mapa creado
def poseOnMap(pose: Pose, map_info: MapMetaData):
    return pose.x < map_info.width and pose.x >= 0 and pose.y < map_info.height and pose.y >= 0

def poseToCell(pose: Pose, map_info: MapMetaData):
    return map_info.width * pose.y + pose.x


class MappingWithKnowPoses(Node):
    def __init__(self, name):
        super().__init__(name)

        self.declare_parameter("width", 50.0)   #50m
        self.declare_parameter("height", 50.0)  #50m
        self.declare_parameter("resolution", 0.1)   #10cm

        width = self.get_parameter("width").value
        height = self.get_parameter("height").value
        resolution = self.get_parameter("resolution").value

        self.map_ = OccupancyGrid()
        self.map_.info.resolution = resolution
        self.map_.info.width = round(width / resolution)
        self.map_.info.height = round (height / resolution)
        self.map_.info.origin.position.x = float(-round(width / 2.0)) #indica la pos 0;0, en este caso el centro
        self.map_.info.origin.position.y = float(-round(height) / 2.0)
        self.map_.header.frame_id = "odom" #seteamos un frame de referencia
        self.map_.data = [-1] *(self.map_.info.width * self.map_.info.height)

        #pub, sub y timer para llamar a un cb que publique cada 1 seg
        self.map_pub = self.create_publisher(OccupancyGrid, "map", 1)
        self.scan_sub = self.create_subscription(LaserScan, "scan", self.scan_callback, 10)
        self.timer = self.create_timer(1.0,self.timer_callback)

        self.tf_buffer = Buffer()
        self.tf_listener = TransformListener(self.tf_buffer, self)

    def scan_callback(self, scan: LaserScan):
        #lee la posición del robot (creo)
        try:
            t = self.tf_buffer.lookup_transform(self.map_.header.frame_id, scan.header.frame_id, rclpy.time.Time())
        except LookupException: 
            self.get_logger().error("No se puede realizar la transformación entre /odom y /base_footprint")
            return 
        
        #hacemos una transformación de posiciones en distintos frames. Y convertimos a occupancy grid
        robot_p = coordinatesToPose(t.transform.translation.x, t.transform.translation.y, self.map_.info)
        if not poseOnMap(robot_p, self.map_.info):
            self.get_logger().error("El robot está fuera del mapa!")
            return 
        
        #marcamos la posición del robot en el mapa de ocupación, indicando las celdas ocupadas por el robot
        robot_cell = poseToCell(robot_p, self.map_.info)
        self.map_.data[robot_cell] = 100


    def timer_callback(self):
        self.map_.header.stamp = self.get_clock().now().to_msg()
        self.map_pub.publish(self.map_)


def main():
    rclpy.init()
    node = MappingWithKnowPoses("mapping_with_know_poses")
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()

if __name__ == "__main__":
    main()